defmodule Pooly.PoolServer do
  ...
  defmodule State do
    defstruct pool_sup: nil, worker_sup: nil, monitors: nil, size: nil, workers: nil, name: nil, mfa: nil, max_overflow: nil
  end
  ...
  # client api functions use name/1 to reference the appropriate pool server
  def start_link(pool_sup, pool_config) do
    pool_config
    |> Enum.each(fn(config) ->
      IO.inspect(config[:name])
      GenServer.start_link(__MODULE__, [pool_sup, config], name: name(config[:name]))
    end)
  end

  def checkout(pool_name) do
    GenServer.cast(name(pool_name), :checkout)
  end
  ...
  def init([pool_sup, pool_config]) when is_pid(pool_sup) do
    ...
    # store the pool Supervisor's pid in the GenServer state
  ...
  # whenever the worker Supervisor exits, it will terminate the pool server for the same reason that it terminated the worker Supervisor
